...
var arrayLocaleInfo = "es-CL|es|CL|1;0;|2|,|.|Enero;Febrero;Marzo;Abril;Mayo;Junio;Julio;Agosto;Septiembre;Octubre;Noviembre;Diciembre|Ene;Feb;Mar;Abr;May;Jun;Jul;Ago;Sep;Oct;Nov;Dic|Domingo;Lunes;Martes;Miércoles;Jueves;Viernes;Sábado|Dom;Lun;Mar;Mié;Jue;Vie;Sáb|dd-mm-yyyy|dddd, dd \de mmmm \de yyyy|H:nn|H:nn:ss|0||".split("|");
...
<!--  funciones agregadas por david gatica  -->
...
function abrirDialogo(sUrl, w, h)
{
	//Este string especifica el comportamiento del Diálogo Modal
	Args = "dialogHeight: "+h+"px; dialogWidth: "+w+"px; edge: Raised; center: Yes; help: No; resizable: No; status: No";
	// Mostramos el Diálogo Modal y enviamos el objeto oSelection
	window.showModalDialog(sUrl, window,  Args)
}


<!-- Funciones Proposito General -->

//Funciones JS Adicionales

function AbrirVentanaPopup(sURL, sNombreVtna, nAncho, nAlto)
{
        ventana = window.open(sURL, sNombreVtna,'resizable=no,scrollbars=no,menubar=no,status=no,width=' + nAncho + ',height=' + nAlto);
		/*scrollbars=no,status=no,width=300,height=300,menubar=no*/
        ventana.focus();
}

function AbrirVentanaPopup_scrolls(sURL, sNombreVtna, nAncho, nAlto)
{
        ventana = window.open(sURL, sNombreVtna,'resizable=no,scrollbars=yes,menubar=no,status=no,width=' + nAncho + ',height=' + nAlto);
		/*scrollbars=no,status=no,width=300,height=300,menubar=no*/
        ventana.focus();
}

function ActualizarPopupDependiente(ctrlPopup, nSeleccion, asPopup)
{
	if(nSeleccion == "" || nSeleccion == null)
		nSeleccion = 0;

	nNroElementos = ctrlPopup.length;
	for(nCont=1; nCont <= nNroElementos; nCont++)
		ctrlPopup.options[0] = null;

	if(asPopup[nSeleccion] != undefined)
	{
		for(nCont=1; nCont <= asPopup[nSeleccion].length; nCont++)
		{
	 		ctrlPopup.options[nCont-1] = asPopup[nSeleccion][nCont-1];
		}

		ctrlPopup.options[0].selected = true;
	}
	else
	{
		ctrlPopup.options[0] = asPopup[0][0];
	}
}

function ActualizarPopupDependiente2(ctrlPopup, nSeleccion, asPopup)
{
	if(nSeleccion == "" || nSeleccion == null)
		nSeleccion = 0;
	...
	if(asPopup[nSeleccion] != undefined)
	{
		...
		{
	 		//ctrlPopup.options[nCont-1] = asPopup[nSeleccion][nCont-1];
			ctrlPopup.options[nCont-1] = asPopup[nSeleccion][nCont];
		}
