...
class clsTemplate
{
  ...
  function __construct($external_encoding = "", $out_encoding = "", $internal_encoding = "")
  {
  ...
  function getname($array_key, $is_block)
  {
    ...
    else if(!isset($searching_array[$array_key]))
    {
      ...
      foreach( $searching_array as $key=>$dontcare) 
      {
        $key_len = strlen($key);
  ...
  function print_block($block_name)
  {
    ...
    echo "<table border=\"1\">";
    foreach( $this->blocks[$block_name] as $key=>$value) 
    {
